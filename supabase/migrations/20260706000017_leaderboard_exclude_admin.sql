-- TIP-099 — Bảng xếp hạng: loại HẲN acc admin (profiles.is_admin = true) khỏi mọi kỳ (tuần/tháng/
-- toàn thời gian) — acc admin chỉ dùng để test/quản trị, không phải người dùng thật, nên không
-- bao giờ được xếp hạng hay ghim riêng (kể cả khi chính admin đang xem trang).
-- MIRROR get_leaderboard() ở 20260705000015, chỉ thêm điều kiện lọc is_admin.
...
begin
  ...
  ranked as (
    -- TIP-099: loại acc admin khỏi ranked ngay từ đầu (không tính vào top, không tính vào
    -- "count(*) from ranked" dùng để gán hạng kế tiếp cho người chưa ranked).
    select a.user_id, a.total_sec,
           row_number() over (order by a.total_sec desc, a.user_id) as rank,
           p.nickname, p.avatar_url
    from agg a
    join public.profiles p on p.id = a.user_id and p.is_admin is not true
  ),
  top as (
    select coalesce(
  ...
  caller as (
    select case
      -- TIP-099: chính người xem là admin → KHÔNG bao giờ ghim/xếp hạng, kể cả 0 phút hay đầy phút.
      when (select coalesce(is_admin, false) from public.profiles where id = v_uid) then null
      ...
      when exists (select 1 from ranked where user_id = v_uid) then (
        ...
        select jsonb_build_object(
          'rank', case when (select count(*) from ranked) < 5 then (select count(*) from ranked) + 1 else null end,
          'user_id', v_uid, 'nickname', p.nickname, 'avatar_url', p.avatar_url, 'minutes', 0)
