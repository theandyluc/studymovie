-- TIP-084 — Bảng xếp hạng: khi user CHƯA có giờ học trong kỳ (không nằm trong "ranked")
-- và tổng số người đã xếp hạng CHƯA đủ 5 (top5 hiện ở FE), gán cho user đó số hạng
-- KẾ TIẾP theo thứ tự (đếm số người đã ranked + 1) thay vì để rank=null (trước đây
-- FE ẩn hẳn số hạng khi rank=null → nhìn như "thiếu" số thứ tự).
-- MIRROR get_leaderboard() ở 20260702000013, chỉ sửa nhánh "caller" cuối cùng.
...
begin
  ...
  caller as (
    select case
      ...
      -- TIP-084: chưa ranked (0 phút) → nếu top CHƯA đủ 5, gán hạng kế tiếp theo thứ tự
      -- (đếm số người đã ranked + 1) để FE luôn hiện được số hạng thay vì trống.
      else (
