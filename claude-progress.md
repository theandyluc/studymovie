...
- **Verification:** backend lint/typecheck/test (44 test, không đổi số lượng) PASS. Extension lint/typecheck/build:prod PASS.
- **Version/nộp store (đổi giữa chừng session):** ban đầu tưởng bản 1.1.2 (Session 43) CHƯA nộp store nên build đè lên cùng zip đó. Khách sau đó xác nhận **ĐÃ NỘP 1.1.2 rồi** (nộp trước khi có fix #8) → không đè được nữa, đã **bump version 1.1.2 → 1.1.3**, build lại + đóng gói `studymovie-extension-v1.1.3-20260709.zip` trên Desktop (xoá zip 1.1.2 cũ). Khách hỏi thêm "nộp 3 lần cùng ngày có sao không" (1.1.0 + 1.1.2 đã ĐƯỢC DUYỆT, sắp nộp 1.1.3 là lần 3) — đã trả lời: Chrome Web Store không giới hạn số lần nộp/ngày, mỗi lần xét độc lập, 2 lần được duyệt là tín hiệu tốt.
- **Còn dở:** (1) khách CHƯA runtime-test lại fix "chỉ 1 nghĩa" (mục 8) trên bản mới; (2) TIP-101-BUGFIXES vẫn `in_progress` — CÒN CẢ fix cache quảng cáo (mục 7, Session 43) khách cũng chưa xác nhận test lại; (3) bản **1.1.3** (gộp cả 2 fix) CHƯA nộp Chrome Web Store — khách sẽ tự nộp sau khi Google duyệt xong 1.1.2 đang chờ.
- **Cách resume:** hỏi khách đã test lại chưa (cả bug quảng cáo mục 7 lẫn "chỉ 1 nghĩa" mục 8) trên bản 1.1.3 → cả 2 OK thì chuyển `TIP-101-BUGFIXES` sang `verified` trong `feature_list.json`, nhắc khách nộp bản 1.1.3. Nếu có lỗi mới, **tra `supabase db query --linked`/`vercel logs --json` TRƯỚC khi đoán** (bài học lặp lại từ Session 43). Kiểm tra `extension/manifest.json` version hiện tại trước khi bump tiếp — đã có tiền lệ đoán nhầm trạng thái "đã nộp/chưa nộp" 1 lần trong chính session này, luôn HỎI LẠI khách cho chắc thay vì suy đoán từ note cũ.
...
- **Danh sách fix (thứ tự phát hiện):**
  1. Tra từ dính dấu câu (`hello,`) — `cleanWord()` đã có nhưng popup TITLE dùng `surface` thô chưa qua clean. Sửa + đổi regex sang `\p{L}` Unicode.
  2. Phiên âm sai đôi lúc — `lemmaCandidates()` heuristic cắt đuôi sinh candidate rác + đúng candidate (`getting`→`get`) bị đẩy xuống cuối rồi bị `slice()` cắt mất; đồng âm khác nghĩa (`lead`, `content`) bị gộp phonetics nhầm entry.
  3. "communication" → chỉ hiện `/co/` — dữ liệu FVDP import cũ bị lỗi (IPA cắt cụt). Thêm `isLikelyTruncatedIpa()` tự vá qua Free Dictionary + ghi đè cache.
  4. Phụ đề mất khi mouse leave lúc popup tra từ mở — **2 lần đoán sai** (z-index YouTube overlay — SAI; ) trước khi tìm ra gốc rễ thật: `onMessage` nhận `SM_CUES` remove+rebuild overlay VÔ ĐIỀU KIỆN mỗi lần buffering nền có cue mới, syncTick bỏ qua vẽ lại vì `popupOpen` → khung trống. Sửa: chỉ rebuild khi cần, luôn vẽ lại cue ngay không phụ thuộc popupOpen.
  5. Độ trễ dịch 2-4s lúc mở video/tua — thử tách "urgent chunk" nhỏ (2 rồi 1 câu) để trả nhanh hơn, khách chỉ ra đang ĐOÁN MÒ (session 42 đã đo 4→2 chậm hơn dù có confound biến số) → **REVERT về đúng CHUNK_COUNT=4 đã verify**, không tối ưu thêm khi chưa có số đo thật.
  6. Tua liên tục làm phụ đề "hiện rồi mất" — race condition thật: nhiều request dịch chạy chồng lấn lúc tua dồn dập, response tới sau (dựa snapshot cũ hơn) ghi đè xoá mất câu response khác vừa dịch xong. Sửa: merge thay vì ghi đè `viArr`.
  7. Phần "không dịch kịp" còn sót (độ trễ AI thật, không phải bug) — thêm placeholder "Đang dịch…" ngay trong dòng phụ đề Việt thay vì để trống, bỏ dòng thông báo vàng trùng lặp.
  8. Phụ đề Anh/Việt quá dài — tưởng do câu Việt dài (SAI, khách sửa lại: câu ANH dài kéo Việt dài theo). Root cause thật: `groupIntoSentences()` chỉ GHÉP cụm ASR, không bao giờ TÁCH — cụm gốc tự nó dài hơn trần thì trần vô tác dụng. Thêm `splitOverlongCue()`.
  9. "it's" hiện nghĩa "nó" — tưởng thiếu ý (sửa AI dịch đủ "nó là"), khách sửa lại: muốn NGẮN GỌN đúng "nó" thôi + muốn tra hẳn "it" (cả IPA). Revert prompt AI, sửa đúng chỗ: `cleanWord()` bóc `'s` cuối từ trước khi gửi tra (KHÔNG bóc `'m/'re/'ve/'ll/'d/'t` — khách xác nhận không mở rộng).
  10. "doesn't" thiếu IPA — từ phủ định rút gọn hầu hết không có trong FVDP lẫn Free Dictionary API. Thêm `NEGATION_CONTRACTION_BASE` (bảng tường minh, bất quy tắc: `can't`→`can`, `won't`→`will`).
  11. Debug sâu hơn phát hiện tự gây bug ở mục 2: `primaryEntry` chọn theo `phonetics.length>0` không đòi hỏi có `text` thật — entry "does" có phonetics chỉ chứa audio (không text) bị chọn nhầm làm chính, bỏ lỡ IPA thật ở entry sau. Sửa điều kiện chọn.
  12. **Bug nghiêm trọng nhất** — video tua qua ~35s thì mất hẳn phụ đề, reload không hết. Debug bằng `supabase db query --linked` trực tiếp vào `vi_caption_cache` (không đoán mò): cache chỉ phủ 0-34.84s, NỘI DUNG LÀ SCRIPT QUẢNG CÁO — xác nhận chắc chắn qua YouTube oEmbed API (video thật "Learn AI Agents in 13 Minutes", cache lưu lại là quảng cáo app học kiểu game, hoàn toàn khác chủ đề). Quảng cáo pre-roll gọi cùng `/api/timedtext` trong khi URL trang không đổi (`?v=` vẫn là video chính) → interceptor bắt nhầm, cache CỨNG vĩnh viễn (baseline chỉ tạo 1 lần/video, không bao giờ tạo lại). Fix 2 lớp chặn (so khớp `v` param URL với `currentVideoId()`, check class `ad-showing` YouTube gắn lên `#movie_player`). Quét toàn bộ `vi_caption_cache` (lọc độ phủ <60s) tìm thêm **4 video khác** dính cùng bug, đọc nội dung xác nhận đều là quảng cáo (Adobe Acrobat Studio, ClickUp Brain, app thanh toán, app học kiểu game) — xoá cả 5 dòng cache hỏng.
- **Bài học lặp lại trong session này (đáng ghi nhớ):** đoán mò dựa suy luận (không đo/không tra dữ liệu thật) sai ít nhất 3 lần (z-index overlay, hướng dịch it's, hướng câu dài EN/VI) — mỗi lần khách sửa lại đúng hướng. Lần tìm bug quảng cáo CHỈ ra được nhờ `supabase db query --linked` tra thẳng dữ liệu thật + đối chiếu oEmbed, không đoán. **Luôn ưu tiên tra dữ liệu/log thật trước khi sửa khi có thể** (đã có sẵn quyền `supabase db query --linked` và `vercel logs --json` dùng được ngay, không cần hỏi khách).
- **Verification:** backend lint/typecheck/test PASS mọi vòng (44 test cuối, thêm mới `lookup.test.ts` +9, `sentence-group.test.ts` +2). Extension lint/typecheck/build/build:prod PASS mọi vòng. Backend tự deploy qua Vercel sau mỗi push (xác nhận qua `vercel ls --prod` Ready). Extension version bump 1.1.1→**1.1.2**, đã đóng gói `studymovie-extension-v1.1.2-20260709.zip` trên Desktop (xoá zip 1.1.1 cũ lỗi thời) — **CHƯA nộp Chrome Web Store**, chờ khách tự nộp.
- **Còn dở:** khách CHƯA runtime-test lại fix cuối (chặn cache quảng cáo) sau khi reload extension — cần xác nhận video `P5sKKnWCvzk` (hoặc video mới) tạo cache đúng, phụ đề khớp xuyên suốt không đứt ở ~35s nữa, TRƯỚC KHI đánh dấu `TIP-101-BUGFIXES` → `verified` trong `feature_list.json` (hiện đang `in_progress`).
- **Cách resume:** đọc feature_list.json entry `TIP-101-BUGFIXES` (evidence đầy đủ từng fix) → hỏi khách đã test lại bug quảng cáo chưa → nếu OK thì chuyển `verified`; nếu khách báo lỗi mới, **tra `supabase db query --linked`/`vercel logs --json` TRƯỚC** khi đoán, theo đúng bài học session này.
...
- **RUNTIME-VERIFY THẬT (khách tự test nhiều vòng trên YouTube thật) — lộ thêm nhiều lỗi CHỈ thấy được khi chạy thật, không phải lint/test:**
  1. **Lô dịch mở màn quá to (50 câu)** → AI trả lời >20s, timeout → "không dịch được phụ đề Việt". Fix: tách `BOOTSTRAP_COUNT` nhỏ (15) rồi 50→15→6→4 (đo thực tế mỗi lần giảm đều nhanh hơn).
  2. **`response_format: json_schema (strict)`** — nghi ngờ + xác nhận qua log Vercel (`vercel logs --json`) là nguyên nhân khiến OpenAI phản hồi chậm bất thường, hay bị "This operation was aborted". Đổi sang `json_object` (nhẹ hơn) — đỡ nhưng CHƯA hết hẳn.
  3. **`region: sin1` (Singapore, đặt ở TIP-101a để tối ưu Supabase)** hoá ra khiến MỌI lệnh gọi OpenAI (máy chủ chính ở Mỹ) tốn thêm độ trễ vòng Singapore↔Mỹ — gỡ hẳn, quay về region mặc định Vercel.
  4. **Lỗi nghiêm trọng nhất, tìm qua `raw response` log:** dù đã dặn rất rõ trong prompt "giữ đúng số lượng bản dịch = số câu input, không gộp", model **RẤT HAY tự ý gộp nhiều câu tiếng Anh thành 1 câu dịch** — log Vercel cho thấy xảy ra ở GẦN NHƯ MỌI lô dịch nhiều câu (không phải hiếm). Đây là giới hạn thật của việc bắt LLM trả về mảng N phần tử khớp đúng N input — không đáng tin cậy dù đã thử structured output, json_object, tự chia đôi lô khi lệch (`translateBatch` đệ quy). **Fix triệt để: đổi hẳn kiến trúc sang dịch TỪNG CÂU RIÊNG, gọi SONG SONG (`Promise.all`)** — loại bỏ hoàn toàn khái niệm "mảng phải khớp số lượng", mỗi câu là 1 lần gọi độc lập trả text thuần (giống mẫu `lookup-context.ts` đã ổn định). Từ đây KHÔNG còn lỗi gộp/lệch câu nữa.
  5. **Câu ghép quá dài** (100/7s rồi 60 ký tự/5s) theo phản hồi khách đọc thấy dài — hạ trần `sentence-group.ts` 2 lần.
  6. **Tối ưu tốc độ sau khi hết lỗi đúng/sai:** gộp 2 lượt gọi mạng nối tiếp (hỏi cache riêng rồi mới dịch) thành 1 lượt duy nhất (xoá hẳn `GET /api/captions-vi`, endpoint dịch tự kiểm tra cache trước khi gọi AI); bỏ chờ đồng bộ (`await`→`waitUntil`) bước ghi dòng cache nền tảng — rủi ro race giảm nhiều vì không còn 2 lượt gọi nối tiếp từ cùng 1 phiên nữa.
  7. **Thử nghiệm CHUNK_COUNT (số câu/lần gọi) qua nhiều vòng đo THỰC TẾ:** 50→15→6→4 đều cải thiện rõ; thử tiếp 4→2 (kèm giảm chu kỳ kiểm tra 4s→2s để bù) lại **ĐO CHẬM HƠN** (10s mở video thay vì 4s, tua 2-4s không ổn định thay vì luôn ~2s) — REVERT về 4 câu/chu kỳ 4s (điểm tối ưu tốt nhất đo được, không phải đoán). Bài học: không phải cứ giảm nhỏ hơn là nhanh hơn, có điểm gãy do tranh chấp/quá tải.
  - **Kết quả cuối, khách xác nhận qua nhiều vòng test thật:** mở video mới ~2s (thiểu số ~4s do OpenAI dao động độ trễ tự nhiên, không phải lỗi hệ thống), tua nhảy ~2s, không còn lỗi dịch sai/gộp câu/mất phụ đề. Khách chốt "ok vậy là đã xong".
- **Verification:** backend lint/typecheck/test (26 test) PASS mọi vòng sửa. Extension lint/typecheck/build/build:prod PASS mọi vòng sửa. Migration verify qua Supabase trực tiếp (`supabase db query --linked`, cả schema lẫn dữ liệu thật của video đã test). **feature_list.json → `"verified"`** (đã runtime-verify thật theo đúng quy ước mục 6 CLAUDE.md, không chỉ lint/test).
- **Việc phát sinh sau khi TIP-101 verified (cùng session):**
  - Khách hỏi ước tính chi phí thật dựa trên dashboard OpenAI (usage thật $0.15/968 request hôm nay) — đã giải thích phần lớn chi phí đó là do TÔI test đi test lại lúc debug (không đại diện chi phí thật), tính lại theo kiến trúc CUỐI CÙNG (dịch song song từng câu) + giá thật gpt-4o-mini (đối chiếu khớp dashboard: $0.15/1M input, $0.60/1M output) → ước tính ~$0.009/video dịch toàn bộ, ~9 USD/1.000 video khác nhau — rẻ, không đáng lo ở quy mô hiện tại.
  - Khách hỏi UI popup lúc hết hạn dùng thử — rà code xác nhận **đã có sẵn** màn "Hết hạn dùng thử" riêng trong popup (`popup.ts`, TIP-053, dựa vào `/api/me` → `is_active`), không phải làm mới. Phát hiện phụ: link nâng cấp trong popup trỏ `studymovie.com/gia` (khách xác nhận không dùng trang đó nữa) — đã sửa trỏ về `${SITE_URL}/thanh-toan` (đồng bộ với overlay YouTube). Lưu ý còn treo (chưa sửa, khách chưa yêu cầu): popup (`/api/me`→`is_active`, dựa bảng `subscriptions`) và overlay YouTube (`/api/access-status`→`has_access`, dựa `profiles.created_at`+24h) dùng 2 nguồn tính hết hạn KHÁC NHAU, về lý thuyết có thể lệch nhau hiếm khi.
  - **Version Chrome Web Store:** khách đã nộp bản **1.1.0** (chứa toàn bộ TIP-101) TRƯỚC KHI có fix link popup ở trên — nên bump tiếp lên **1.1.1** (đã build:prod + zip `studymovie-extension-v1.1.1-20260709.zip` trên Desktop) cho lần nộp KẾ TIẾP, sau khi Google duyệt xong bản 1.1.0. KHÔNG tự ý nộp lên store (chỉ đóng gói sẵn) — chờ khách tự bấm "Upload new package".
- **Còn dở (Homeowner action, KHÔNG phải kỹ thuật):** (1) chờ Google duyệt bản 1.1.0 đã nộp, sau đó tự nộp tiếp bản 1.1.1 đã đóng gói sẵn trên Desktop; (2) các việc CHỦ ĐỘNG để sau (đã ghi trong plan, chưa cấp bách): khoá chống đua race cache hiếm gặp, job retry câu lỗi dịch, tính năng lộ trình soạn sẵn + pre-warm, đồng bộ 2 nguồn tính hết hạn (popup vs YouTube overlay) nếu khách thấy cần.
- **Cách resume:** nếu khách báo lỗi runtime mới, đọc lại 7 mục lỗi đã tìm ra ở trên trước khi đoán mò — đa số lỗi lần này CHỈ lộ ra khi chạy thật (không phải lint/test bắt được), nên **luôn dùng `vercel logs <url> --json | grep captions-translate`** để xem log thật (kể cả `console.warn` đã thêm trong `translate-batch.ts`) trước khi sửa mù. Đọc thêm plan đầy đủ ở `/Users/khoai/.claude/plans/humble-tickling-octopus.md` để hiểu thiết kế tổng thể (bám tiến độ xem, không dịch trước cả video, dịch từng câu song song).
...
- **Đã thảo luận + CHỐT hướng đi (khách duyệt), CHƯA CODE:**
  1. **Bỏ hẳn phụ thuộc `tlang` của YouTube**, tự làm pipeline: ghép cụm ASR rời rạc thành câu hoàn chỉnh → dịch câu đó bằng **GPT-4o-mini** (tái dùng key/model đã có ở TIP-038, KHÔNG cần tích hợp Google Translate/DeepL — rẻ hơn ~100 lần nhờ hạ tầng sẵn có, ước tính ~$0.001-0.002/video) → cache theo video (tái dùng ý tưởng cache như `ai_context_meaning`).
  2. **Áp dụng cho MỌI video** (không chỉ giới hạn ở lộ trình soạn sẵn) — khách xác nhận: "học viên xem video khác thì vẫn cứ gọi API dịch trả phí, để tránh bị hạn chế phụ đề". Video trong lộ trình (roadmap khách tự soạn, đa số học viên học theo) nên dịch trước (pre-warm cache, admin trigger khi thêm video vào lộ trình) để không ai phải chờ ở lượt xem đầu; video ngoài lộ trình (thiểu số, học viên tự chọn xem) dịch tại lúc xem (cache-miss thật, chấp nhận có độ trễ).
  3. **Model:** giữ `gpt-4o-mini` (biến `OPENAI_MODEL` đã có, đổi được không cần sửa code) — không đổi sang Google Translate/DeepL vì tốn công tích hợp mới trong khi chi phí đã rất thấp, và LLM hiểu ngữ cảnh/idiom tốt hơn máy dịch câu-đơn-lẻ.
  4. Đã ghi quyết định đảo D-1 vào `StudyMovie-Blueprint.md` mục "CHECKPOINT" + mục 0 (giống cách D-2 từng đảo có kiểm soát ở TIP-038) — đánh dấu **CHƯA triển khai**, chỉ mới chốt hướng.
- **Thảo luận phụ (bản quyền, quan trọng, không phải kỹ thuật):** khách định lấy video "English conversation" dựng từ sách cũ đã ngừng bán để làm lộ trình. Đã tư vấn: (a) ngừng bán ≠ hết bản quyền (thời hạn bảo hộ theo luật, không phụ thuộc việc còn kinh doanh), (b) video còn tồn tại trên YouTube nhiều năm ≠ được chủ sở hữu cho phép (Content ID không phủ hết nội dung cũ/ít người theo dõi, chủ sở hữu phải tự report thì mới bị gỡ), (c) rủi ro cao hơn nếu CHỦ ĐỘNG chọn lọc + thu phí dựa trên nội dung biết rõ nguồn gốc không rõ ràng, kể cả Chrome Web Store cũng có chính sách cấm tiếp tay vi phạm bản quyền. Gợi ý an toàn: kênh chính thức/CC (VOA/BBC Learning English...) hoặc xin phép chủ sở hữu hiện tại. **Khách CHƯA chốt danh sách video cụ thể** — cần khách tự rà lại nguồn trước khi đưa vào lộ trình.
- **Phát hiện phụ (bug thật, chưa sửa):** tra nghĩa từ theo ngữ cảnh (`/api/lookup-context`, TIP-038) đang CHẬM ở lượt tra từ MỚI (cache-miss) vì 2 nguyên nhân trong code:
  1. Bước ghi cache (`sb.from("ai_context_meaning").upsert(...)`, dòng ~82-85 trong `web/backend/src/api/lookup-context.ts`) đang bị `await` TRƯỚC KHI trả response — chặn người dùng chờ thêm 1 round-trip Supabase không cần thiết (ghi cache lỗi cũng chỉ log warning, không ảnh hưởng kết quả trả về).
  2. Backend Vercel KHÔNG set `regions` trong `web/backend/vercel.json` (mặc định vùng Mỹ) trong khi Supabase ở Singapore → mỗi lượt gọi Supabase (đọc cache + ghi cache = 2 lần) phải đi vòng Mỹ↔Singapore, cộng thêm thời gian OpenAI xử lý → cache-miss dễ mất 1-2 giây.
- **VIỆC CẦN LÀM SESSION SAU (khách yêu cầu note lại, hết session limit, CHƯA CODE GÌ HÔM NAY):**
  1. Fix tốc độ `/api/lookup-context`: bỏ `await` ở bước upsert cache (fire-and-forget, trả response ngay sau khi có nghĩa AI), và thêm `"regions": ["sin1"]` vào `web/backend/vercel.json` để gần Supabase Singapore hơn.
  2. Triển khai pipeline dịch phụ đề bằng GPT-4o-mini theo đúng hướng đã chốt ở trên (thay thế hoàn toàn `tlang`) — cần thiết kế: thuật toán ghép cụm ASR→câu, endpoint backend dịch (giống cấu trúc `lookup-context.ts`), bảng cache mới (migration, tương tự `ai_context_meaning` nhưng theo video+segment), sửa `extension/src/content/yt-intercept.ts` để gọi pipeline mới thay vì fetch `tlang=vi` trực tiếp từ YouTube. Đây là task LỚN, nên tách TIP riêng, làm từng bước theo Session Lifecycle (đọc state → chọn 1 việc → verify → ghi state), KHÔNG làm chung 1 lần với việc fix tốc độ ở trên.
- **Cách resume:** đọc lại 2 mục "VIỆC CẦN LÀM" ở trên, làm mục 1 (fix nhanh) trước vì nhỏ + độc lập, xong mới sang mục 2 (pipeline dịch, việc lớn — nên viết TIP-101 rõ acceptance criteria trước khi code, theo đúng Vibecode Kit).

### Session 40 — TIP-098/098b/100: fix 2 lần Chrome Web Store từ chối quyền + mật khẩu phụ trang /admin (2026-07-07)
- **Bối cảnh:** khách tự nộp extension lên Chrome Web Store (đã hướng dẫn ở Session 39), bị Google từ chối tự động 2 lần liên tiếp vì khai quyền không dùng tới.
- **Đã làm:**
  - **Rejection #1 (violation "Purple Potassium", quyền `scripting`):** grep xác nhận không có `chrome.scripting.*` nào trong `extension/src` (mọi content script khai TĨNH qua `content_scripts` trong manifest, không cần quyền `scripting` — quyền đó chỉ cần khi gọi `chrome.scripting.executeScript/insertCSS` lúc runtime). Gỡ `"scripting"` khỏi `manifest.json`, build:prod PASS, cập nhật `HANDOVER.md` (bảng permission-justification), rebuild zip → Desktop. Commit `32e6b67`.
  - **Rejection #2 (quyền `tabs`):** grep xác nhận 3 chỗ dùng `chrome.tabs.*` (`popup.ts`: `tabs.create({url})`; `service-worker.ts`: `tabs.query({})` chỉ đọc `.id`, `tabs.sendMessage()`) đều KHÔNG cần quyền `tabs` theo model quyền thật của Chrome (quyền đó chỉ gate truy cập thuộc tính nhạy cảm như `.url`/`.title`/`.favIconUrl` hoặc query lọc theo url). Gỡ `"tabs"` khỏi manifest, build:prod PASS, cập nhật `HANDOVER.md`, rebuild zip → Desktop. Commit `c8c4441`.
  - **Khách hỏi phòng ngừa "còn chỗ nào bị từ chối nữa không":** rà thêm các API nhạy cảm khác (`cookies`, `webRequest`, `declarativeNetRequest`, `history`, `bookmarks`, `downloads`, `geolocation`, `notifications`, `identity`) — 0 kết quả, không có nguy cơ khác từ góc quyền API. Phát hiện phụ: trang `/privacy` (public) vẫn mô tả quyền `tabs`/`scripting` đã gỡ — sửa đồng bộ lại đúng 4 quyền hiện tại (`storage`, host youtube.com, `alarms`, host app.studymovie.com), lint/typecheck/build PASS, push + Vercel Ready + curl production xác nhận nội dung mới. Commit `81fce2f`.
  - **Điểm CHƯA đụng vào (cố ý, có rủi ro):** `host_permissions` (`*://*.youtube.com/*`, `https://app.studymovie.com/*`) về lý thuyết có thể cũng bị coi dư thừa vì `content_scripts.matches` tự đủ quyền chèn script độc lập với `host_permissions` (MV3). Không gỡ vì chưa đủ tin cậy Google sẽ chấm đây là lỗi (khác hẳn 2 lần trước — hoàn toàn không có code gọi API), và gỡ nhầm có thể làm hỏng tính năng phụ đề. Nếu bị từ chối lần 3 vì lý do host permission, xử lý tiếp.
  - **Mật khẩu phụ trang `/admin` (khách chủ động yêu cầu, sợ người khác dùng chung máy chỉnh nhầm):** thêm endpoint backend `POST /api/admin/verify-page-password` (gate `requireCallerAdmin()` trước như mọi admin action khác, rồi mới so khớp với biến môi trường `ADMIN_PAGE_PASSWORD` — mật khẩu KHÔNG bao giờ nằm trong bundle frontend). Frontend: component `AdminPasswordGate.tsx` bọc `AdminInner` (trong `AuthGuard`), khoá theo `sessionStorage` — mở tab/trình duyệt mới phải nhập lại, mở lại trong cùng tab thì không hỏi nữa. Mật khẩu thật (`Khongcanbiet1$`) đã set qua `vercel env add` vào project backend (production, sensitive), KHÔNG commit vào git.
- **Verification:** extension lint/typecheck/build:prod PASS cả 2 lần gỡ quyền. Backend lint/typecheck/test PASS (endpoint mật khẩu). Frontend lint/typecheck/build PASS (trang /privacy + AdminPasswordGate). Production: `vercel ls` xác nhận deployment Ready cho từng commit; `curl` xác nhận `/privacy` nội dung mới, `/api/admin/verify-page-password` trả 401 khi không có Bearer token (đúng thiết kế — gate xác thực chạy trước gate mật khẩu).
- **Còn dở / chưa verify:** khách cần tự **upload lại zip mới** (`studymovie-extension-v1.0.0-20260706.zip` trên Desktop, đã phản ánh cả 2 lần gỡ quyền) lên Chrome Web Store Dashboard → "Upload new package" → Submit for review lại, nếu chưa làm sau lần rejection #2. Trạng thái review lần nộp lại chưa biết.
- **Quyết định mới:** không có thay đổi kiến trúc chốt (Blueprint mục 0/10) — chỉ là hardening quyền extension + thêm lớp bảo vệ UX cho /admin.
- **Cách resume:** nếu khách báo Chrome Web Store từ chối lần nữa, đọc lý do cụ thể trước — nếu là `host_permissions`, xem điểm "CHƯA đụng vào" ở trên để cân nhắc gỡ có kiểm soát (test kỹ tính năng phụ đề trước khi gỡ). Nếu khách quên mật khẩu trang admin, mật khẩu hiện tại là `Khongcanbiet1$` (đổi qua `vercel env rm/add ADMIN_PAGE_PASSWORD` trên project backend rồi redeploy).

### Session 39 — TIP-095→099: popup tra từ + admin tạo/xoá tài khoản + loại admin khỏi leaderboard + favicon (2026-07-06)
- **Bối cảnh:** tiếp nối Session 38 (vẫn phong cách khách chỉnh tay từng chi tiết qua nhiều lượt phản hồi), mở rộng sang: popup tra từ trên YouTube, trang admin, và dọn nốt các text/màu rải rác.
- **Đã làm:**
  - **Popup tra từ (`youtube.ts`)**: bỏ nút "🔊 Phát âm" dạng text, thay icon loa SVG cạnh từ (căn baseline); chỉ còn 2 nút Lưu (nền xanh khớp badge "Đã học")/Đóng (nền đỏ khớp badge "Từ mới"); dòng phiên âm chỉ hiện `/ipa/` (bỏ lemma đi kèm); font/màu 3 dòng (EN/phiên âm/nghĩa VI) đồng bộ page hoc-tu-vung.
  - **Icon extension** (16/48/128): đổi logo "SM." text cũ sang icon "film-open-star" (đồng bộ web), nền trắng bo góc — render qua Playwright headless (dùng `playwright-core` + Chrome for Testing đã cache sẵn từ `npx playwright`) trực tiếp từ SVG dùng ở web, KHÔNG phải ảnh vẽ tay.
  - **Trang `/admin` — tính năng mới:**
    - `POST /api/admin/create-user` + `DELETE /api/admin/users/:id`: dùng Supabase Admin Auth API (service_role, chỉ backend), gate bằng đúng RPC `is_caller_admin()` có sẵn (không phải admin thì service_role cũng không chạy).
    - Migration `20260706000016`: `admin_get_stats()` thêm `daily_new_users` (7 ngày, giờ VN) → card "Tổng user" đổi thành biểu đồ cột (style `LearnedChart` /tu-vung) + thêm `CircleStat` tổng số user cạnh biểu đồ.
    - UI: card "Tạo tài khoản" (email+mật khẩu) + cột "Xoá" trong bảng user (có `confirmDialog`).
  - **Bảng xếp hạng — migration `20260706000017`:** loại HẲN acc admin (`is_admin=true`) khỏi mọi kỳ (Tuần/Tháng/Toàn thời gian), kể cả khi chính admin đang xem trang. Phát hiện phụ: khách đã tự dùng tính năng xoá tài khoản vừa build để dọn 2 acc trùng tên "Trường Lục Nam" (từng gây hiểu lầm ghi ở Session 38) — nên leaderboard tạm trống mọi kỳ vì người duy nhất từng học chính là acc admin, nay bị loại đúng thiết kế mới.
  - **Favicon web app**: `app/icon.png` (512×512, PNG alpha, HÌNH TRÒN nền trắng — khác extension bo góc vuông, cố ý giữ khác biệt vì quy ước icon extension trên toolbar Chrome thường vuông/squircle, không nên đổi tròn — đã tư vấn khách và khách đồng ý giữ nguyên extension). Next.js App Router tự nhận diện convention `app/icon.png`.
  - **Polish text rải rác** (không đổi logic, chỉ font/màu/spacing theo yêu cầu khách): tất cả "Đang tải" (PageLoading, /tien-do-hoc, LeaderboardCard, WeeklyPlan) → Inter regular #1f1f1f -3%; `/thanh-toan` (màn hết hạn trial gộp vào 1 Card, bỏ dòng "Mở khoá toàn bộ...", thêm emoji ⏳, nút "Nâng cấp ngay"/"Vào học" nền `rgba(31,31,31,0.9)` qua inline style để thắng cascade `bg-primary` mặc định của `Button` dùng chung); `/cam-on`; trang chủ `/`; nút "Lưu" chọn level lần đầu ở `/tien-do-hoc`; text "Chưa có từ nào..." ở `/tu-vung`.
  - **Hỗ trợ nộp Chrome Web Store** (không phải code — tư vấn quy trình): build:prod + đóng gói zip, soạn sẵn mô tả/Single purpose/permission justification tiếng Anh, giải thích "Are you using remote code?" (xác nhận KHÔNG — không eval/import động/script ngoài), gợi ý data usage checkbox theo đúng thực tế code, hướng dẫn tạo tài khoản test qua chính tính năng "Tạo tài khoản" vừa build cho mục Test instructions.
- **Verification:** lint/typecheck/build (frontend) + lint/typecheck/test (backend) + lint/typecheck/build+build:prod (extension) PASS ở mọi bước. 3 migration (`20260706000016`, `20260706000017`, và `20260705000015` từ session trước) đã `supabase db push` + verify trực tiếp bằng `supabase db query --linked` trên production (không phải giả lập). Mọi commit đã push + Vercel deploy Ready + curl production 200 OK xác nhận từng lần.
- **Còn dở / chưa verify:** Chrome Web Store submission — khách đang tự nộp (tôi hỗ trợ tư vấn nội dung, không có quyền bấm Submit thay); trạng thái review chưa biết (in-depth review vì có host permission, có thể mất vài ngày). Extension code đã push main nhưng **CHƯA chắc bản khách vừa nộp là bản build mới nhất** — nếu review xong mà khách phát hiện thiếu gì, cần build:prod + zip lại + tăng version + upload lại (không phải "New item" mới).
- **Cách resume:** đọc lại "Đã làm" ở trên để biết style/spacing hiện tại của popup tra từ + trang admin. Nếu khách hỏi tiếp về Chrome Web Store, kiểm tra trạng thái review trước (Dashboard → Status). Nếu cần polish thêm UI, tiếp tục pattern: sửa → lint/typecheck/build → (nếu web) restart dev server + Vercel deploy; (nếu extension) build:prod + nhắc khách reload unpacked + F5 tab YouTube.
- **Commit:** 9c2f043 (icon+popup tra từ), 8041a57 (admin accounts+biểu đồ+leaderboard style), 1c274af (Đang tải+hết hạn trial+cảm ơn), 80dc12e (favicon+text trang chủ), 9eb22af (nút Lưu level), 1e3519c (email liên hệ), 3836286 (loại admin khỏi leaderboard, DB-only), 684488b (text Chưa có từ nào).
