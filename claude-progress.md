...
- **Còn dở (Homeowner action, KHÔNG phải kỹ thuật):** (1) build:prod + đóng gói zip mới + nộp lại Chrome Web Store — content script (`yt-intercept.ts`, `youtube.ts`) thay đổi rất lớn lần này so với bản đã publish, bắt buộc phải nộp bản mới; (2) các việc CHỦ ĐỘNG để sau (đã ghi trong plan, chưa cấp bách): khoá chống đua race cache hiếm gặp, job retry câu lỗi dịch, tính năng lộ trình soạn sẵn + pre-warm.
